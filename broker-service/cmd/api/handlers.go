...
func (app *Config) sendMail(w http.ResponseWriter, msg MailPayload) {
	...
	//cal mail service
	mailServiceURL := "http://mail-service/send" //mail-service ini disesuaikan dengan nama service yang ada di docker compose yml

	//post to mail service
	request, err := http.NewRequest("POST", mailServiceURL, bytes.NewReader([]byte(jsonData)))
	if err != nil {
		app.errorJSON(w, err)
		return
	}
	...
	//make sure get back status code
	if response.StatusCode != http.StatusAccepted {
...
func (app *Config) logEventViaRabbit(w http.ResponseWriter, l LogPayload) {
	...
	var payload jsonResponse
	payload.Error = false
	payload.Message = "Logged in via Rabbit"
	app.writeJSON(w, http.StatusAccepted, payload)
